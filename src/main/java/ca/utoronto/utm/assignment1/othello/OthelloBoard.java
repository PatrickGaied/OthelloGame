...
public class OthelloBoard {
    ...
    private int flip(int row, int col, int drow, int dcol, char player) {
        char other_player = otherPlayer(player);
        int count = 0;
        while (validCoordinate(row, col) && get(row, col) == other_player) {
     ...
     */
    public char hasMove() {
        boolean P1_valid = false;
        boolean P2_valid = false;
        ...
        int count = (P1_valid ? 1 : 0) + (P2_valid ? 1 : 0);
        if (count == 2) {
            return BOTH;
        }
        if (P1_valid) {
    ...
    public boolean move(int row, int col, char player) {
        ...
        for (int di : directions) {
            for (int dj : directions) {
                ...
                if (P == player) {
                    int count = flip(row + di, col + dj, di, dj, player);
                    if (count != -1) {
                        valid = true;
                    }
                }
